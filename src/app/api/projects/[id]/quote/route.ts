...
export async function POST(
  ...
  try {
    ...
    // Create a notification message in the project
    await supabase
      .from('project_messages')
      .insert({
        project_id: id,
        sender_id: user.id,
        message: `A quote of $${validatedData.amount.toLocaleString()} has been prepared for your project. Please review and respond to proceed.`,
        is_internal: false,
      })
    ...
    if (action === 'accept') {
      ...
      const { data: invoice } = await supabase
        ...
        .insert({
          ...
          amount: quote.amount,
          tax_amount: quote.amount * 0.09, // 9% GST
          total_amount: quote.amount * 1.09,
      ...
      await supabase
        ...
        .insert({
          project_id: id,
          sender_id: user.id,
          message: 'Quote accepted! The project is now in progress. An invoice has been generated for your records.',
          is_internal: false,
        })
      ...
      await supabase
        ...
        .insert({
          project_id: id,
          sender_id: user.id,
          message: 'Quote rejected. Please prepare a revised quote based on my requirements.',
          is_internal: false,
        })
